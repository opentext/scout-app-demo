(function () {
    ...
    function Expedition($q, $http, Blob, $auth, $rootScope, $csDocument) {
        ...
        document.addEventListener('complete', evalFnFromOfflineEvent);
        document.addEventListener('start', evalFnFromOfflineEvent);
        ...
        function complete(completedExpedition) {
            var promise = $q.defer(),
                ...
                auth = new Appworks.Auth(function (data) {
                    uploadPending();
                    ...
                    $rootScope.$on('Asset.uploadPendingImages.complete', function () {
                        submitExpedition(data.authData);
                    });
                });
            ...
            if (offlineManager.networkStatus().online) {
                ...
                offlineManager.defer('complete', arguments);
            }
            ...
            function submitExpedition(configuration) {
                url = configuration.gatewayUrl + $rootScope.scoutServicePath;
                config.headers.otdsticket = configuration.otdsticket;
        ...
        function save() {
            ...
            cache.setItem(STORAGE_KEY, JSON.stringify(self.expeditions));
        ...
        function generateUrl(nodeId, addVersion, config) {
            var url = config.gatewayUrl + $rootScope.contentServicePath + nodeId;
            if (addVersion) {
                url += '/content';
            } else {
                url += '/children';
        ...
        function generateUploadReq(file, name, config) {
            ...
            return {
                options: {
                    headers: {
                        ...
                        'otcsticket': config.authResponse.addtl['otsync-connector'].otcsticket
                    },
        ...
        function init() {
            var cache = new Appworks.AWCache();
            self.expeditions = JSON.parse(cache.getItem(STORAGE_KEY));
        ...
        function createObject(obj, nodeId, update) {
            var promise = $q.defer(),
                ...
                offlineManager = new Appworks.AWOfflineManager(),
                auth = new Appworks.Auth(function (data) {
                    url = generateUrl(nodeId, update, data.authData);
                    req = generateUploadReq(blob, null, data.authData);
        ...
        function startExpeditionWorkflow(expedition) {
            var promise = $q.defer(),
                ...
                auth = new Appworks.Auth(function (data) {
                    console.log(data);
                    var authResponse = data.authData,
                        url = authResponse.gatewayUrl + $rootScope.scoutServicePath,
                        config = {headers: {otdsticket: authResponse.otdsticket}},
                        request = generateWorkflowReq(expedition, authResponse.authResponse.addtl['otsync-connector']);
            ...
            if (offlineManager.networkStatus().online) {
                ...
                offlineManager.defer('start', arguments);
                promise.resolve(expedition);
            }
